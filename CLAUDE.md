...
- **vault.lua** — Wraps the `ansible-vault` CLI. Encrypt/decrypt work via temp files (`vim.fn.tempname()`). Encrypt passes `--output` to a second temp file so ansible never shreds the plaintext in place; its fallback shredder divides by zero on 1-byte files when `shred` fails. The plugin zero-fills and removes the plaintext file itself. Detects encrypted text by checking for `!vault |` (inline YAML) or `$ANSIBLE_VAULT;` header. `format_inline()` converts full vault output to YAML inline format (`!vault |` with 2-space indented lines).
